...
@WebServlet("/ProjectRegistServlet")
public class ProjectRegistServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			int result = new ProjectDAO().registproject(companyCode, projectCode, projectName, division, projectSince, projectTill, contractCompany, startDate, costDivision, projectType, projectSum, projectGroup, note, employmentManager, employmentManagerIdentityNum, EMposition, EMtask, employment);
			if (result == 1) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
