...
(function($) {
    ...
    $.fn.postcodify = function(options) {
        ...
        return this.each(function() {
            ...
            var keywordInput = $('<input type="text" class="form-control keyword" style="width: 75%; float: left;" value="" />').attr("id", uniqueId).appendTo(controls);
            var searchButton = $('<button type="button" class="btn btn-primary search_button" style="float: right;"></button>').attr("id", uniqueId + "_button").html(settings.searchButtonContent).appendTo(controls);
            ...
            searchButton.click(function(event) {
                ...
                if (keywords.length < 3) {
                    if (settings.useAlert) {
                        ...
                        $('<div class="too_short" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status").html(info.translations[settings.language].errorTooShort.replace(/\n/g, "<br>")).appendTo(results);
                    }
                    return;
                }
                ...
                if (settings.requireExactQuery) {
                    if (!keywords.match(/(사서함|[동리가로길])\s*([0-9]+)(-[0-9]+)?(번지?)?($|,|\s)/) &&
                        ...
                        if (settings.useAlert) {
                            ...
                            $('<div class="too_short" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status").html(info.translations[settings.language].errorExactQuery.replace(/\n/g, "<br>")).appendTo(results);
                        }
                        return;
                    }
                }
                ...
                ajaxSuccess = function(data, textStatus, jqXHR) {
                    ...
                    // 무료 API 서버의 일일 검색 허용 횟수를 초과한 경우...
                        ...
                        if (settings.useAlert) {
                            ...
                            err = $('<div class="quota" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status");
                    ...
                    // 그 밖의 에러 발생시...
                        ...
                        if (settings.useAlert) {
                            ...
                            err = $('<div class="error" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status");
                    ...
                    // 정상 처리되었지만 검색 결과가 없는 경우...
                        ...
                        if (settings.useAlert) {
                            ...
                            err = $('<div class="empty" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status");
                    ...
                    // 검색 결과가 있는 경우...
                        ...
                        if (data.count >= 100) {
                            if (settings.useAlert) {
                                ...
                                err = $('<div class="too_many" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status");
                    ...
                    if (typeof data.msg !== "undefined" && data.msg !== "") {
                        var msg = $('<div class="message" style="margin-top: 50px;"></div>').postcodifyAddClass("search_status");
                        msg.text(data.msg);
